...
left_join(flights2, airlines) #same as right_join(airlines, flights2)
```
...
``` r
#intersect(y,x) #both datasets must have identical columns
```
...
### 2. Can we apply `set` and `binding` functions between `flights2` and `weather` datasets. Why and why not?

``` r
#union(flights2, weather) #can't do
...
``` r
#bind_cols(flights2, weather) #can't because different number of rows
```
