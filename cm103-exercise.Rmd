...
str_c(person, " takes ", commute, " minutes to commute to work.") %vectorised
```
...
map(got_chars, `[`, desired_info) %>% head(3)
...
```{r}
got_chars %>% {  #curly brackets for piping, check ?`{` - the next function is {, not tibble
    tibble(
        name = map_chr(., "name"),
        born = map_chr(., "born")
    )
